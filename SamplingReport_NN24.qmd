...
target_crs <- 3995 # EPSG:3995 for ETRS89 / NTM zone 4

# Load Norway shapefile from rnaturalearth
norway <- ne_states(country = c("norway", "sweden", "finland"), returnclass = "sf")
norway <- st_crop(norway, xmin = 4.3, xmax = 32, ymin = 55.5, ymax = 71.3) # Crop to the area of interest

# ---- Reproject 'norway' to EPSG:3995 first ----
norway_projected <- st_transform(norway, crs = target_crs)

# Create a bounding box for Finnmark in WGS84 (EPSG:4326)
finnmark_bbox_wgs84 <- st_sfc(st_polygon(list(matrix(
  c(25, 69.5,
    32, 69.5,
    32, 71.5,
    25, 71.5,
    25, 69.5), # Close the polygon
  ncol = 2, byrow = TRUE
))), crs = 4326)

# Transform the Finnmark bounding box polygon to the target CRS (EPSG:3995)
finnmark_crop_area_projected <- st_transform(finnmark_bbox_wgs84, crs = target_crs)
finnmark <- st_crop(norway_projected, finnmark_crop_area_projected)

shp <- read.csv("files/site_coords.csv")
shp <- st_as_sf(shp, coords = c("X", "Y"), crs = 4326) # Initial CRS is WGS84
shp <- st_transform(shp, crs = target_crs) # Transform to EPSG:3995 (Arctic Stereographic)
...
finnmark_map <- ggplot() +
  ...
  geom_sf_text(data = shp, aes(label = name), color = "black",
               size = 3, nudge_x = 5000, nudge_y = 5000, # Nudge values will now be in meters
               fontface = "bold.italic") +
  theme_minimal() +
  # Ensure the plot uses the target CRS for proper rendering and labels
  coord_sf(crs = target_crs) # Explicitly set coord_sf to the target_crs

# Create the inset map of mainland Norway
norway_map <- ggplot() +
  geom_sf(data = norway_projected, fill = "lightgrey", color = "darkgrey") + 
  geom_sf(data = finnmark_bbox_wgs84, fill = NA, color = "red", size = 0.5) +
  # suppress automatic extension of axes
  coord_sf(expand = FALSE) + theme_void()

# Use cowplot to create an inset
combined_map <- ggdraw() +
  draw_plot(finnmark_map) +
  draw_plot(norway_map, x = 0.35, y = 0.22, width = 0.3, height = 0.3) # Example values
